...
function handle_cluster_click(self, cluster, release) {
  ...
  if (cluster) {
    menu_object
      ...
      .append("a")
      .attr("tabindex", "-1")
      .text(__("network_tab")["expand_cluster"] || "Expand cluster")
      .on("click", (d) => {
        cluster.fixed = 0;
    ...
    menu_object
      ...
      .append("a")
      .attr("tabindex", "-1")
      .text(__("network_tab")["center_on_screen"] || "Center on screen")
      .on("click", (d) => {
        cluster.fixed = 0;
    ...
    menu_object
      ...
      .attr("tabindex", "-1")
      .text((d) => {
        if (cluster.fixed) return __("network_tab")["allow_float"] || "Allow cluster to float";
        return __("network_tab")["hold_position"] || "Hold cluster at current position";
      })
      .on("click", (d) => {
    ...
    if (self.isPrimaryGraph) {
      menu_object
        ...
        .append("a")
        .attr("tabindex", "-1")
        .text((d) => __("network_tab")["show_separate_tab"] || "Show this cluster in separate tab")
        .on("click", (d) => {
    ...
    if (clustersOfInterest.get_editor()) {
      menu_object
        ...
        .append("a")
        .attr("tabindex", "-1")
        .text((d) => __("network_tab")["add_to_cluster_interest"] || "Add this cluster to the cluster of interest")
        .on("click", (d) => {
          clustersOfInterest
            .get_editor()
    ...
    if (show_on_map_enabled) {
      menu_object
        ...
        .append("a")
        .attr("tabindex", "-1")
        .text(__("network_tab")["show_on_map"] || "Show on map")
        .on("click", (d) => {
          //console.log(cluster)
          self.open_exclusive_tab_view(
            cluster.cluster_id,
